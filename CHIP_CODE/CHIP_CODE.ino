...
// Passwords are now obfuscated and retrieved via getAuthPassword() and getDevPassword()
inline String getAuthPassword() {
    char p[] = { 'S','E','R','V','O','_','E','V','D','R','_','2','0','2','6','\0' };
    return String(p);
}
inline String getDevPassword() {
    char p[] = { 'S','E','R','V','O','_','D','E','V','_','2','0','2','6','\0' };
    return String(p);
}
...
__attribute__((used)) static const char FW_IMAGE_IDENTITY_BLOCK[] ="FW_META|FW_MODEL=" FW_VARIANT "|FW_PROJECT=FINAL_BLE_UART_CODE|";
...
namespace Pins {
    ...
    const uint8_t MCU_D2 = 2;
    const uint8_t MCU_D1 = 3;
    const uint8_t MCU_D0 = 18;
    const uint8_t SAT_LED = 8;
...
namespace LedConfig {
    // Normal saturation pattern: 1000ms ON, 1000ms OFF
    const unsigned long STATUS_LED_ON_MS = 1000UL;
    const unsigned long STATUS_LED_OFF_MS = 1000UL;
}
...
static const int MAX_BREAKPOINTS = 10;

Adafruit_NeoPixel statusNeoPixel(1, Pins::STATUS_LED, NEO_GRB + NEO_KHZ800);
unsigned long bleBluePulseStartMs = 0;
unsigned long bleBlueLastCycleMs = 0;
bool bleBluePulseActive = false;
uint32_t currentNeoColor = 0;
...
struct Configuration {
    ...
    int bp_count;
    float bp_in[MAX_BREAKPOINTS];
    float bp_out[MAX_BREAKPOINTS];
    float Kp, Ki;
...
struct RuntimeData {
    ...
    unsigned long lastLedUpdateUs;
    float led_breath_phase;
    String inputString;
};
...
void anchorFirmwareIdentityBlock();
...
String detectDeviceVariant() {
    ...
    dn.trim();
    dn.toUpperCase();
    return dn.startsWith(String(FW_VARIANT) + "_") ? String(FW_VARIANT) : "";
}
...
volatile uint8_t fwIdentityAnchor = 0;

void anchorFirmwareIdentityBlock() {
    // Read identity bytes via volatile pointer so linker keeps this block in final BIN.
    const volatile char *identity = FW_IMAGE_IDENTITY_BLOCK;
    uint8_t checksum = 0;
    for (size_t i = 0; identity[i] != '\0'; ++i) {
        checksum ^= static_cast<uint8_t>(identity[i]);
    }
    fwIdentityAnchor = checksum;
}
...
bool handleDeveloperControlCommand(const String &cmd, String &response, DevCommandSource source) {
    ...
    if (upper.startsWith("FW_BEGIN:")) {
        ...
        if (requestedVariant != String(FW_VARIANT)) {
        ...
        if (deviceVariant != String(FW_VARIANT)) {
        ...
        touchDeveloperSession();
        response = String("FW_READY:") +
                   String((unsigned long)fwExpectedSizeBytes) +
                   "|PROTO:V1.9_BLE_OTA_FAST";
        return true;
    }
...
void parseAndApplyConfig(String json) {
    ...
    if (doc.containsKey("input") && doc["input"].is<JsonArray>()) {
        JsonArray inputArr = doc["input"];
        if (inputArr.size() >= 2) {
            settings.scale_in_min = inputArr[0];
            settings.scale_in_max = inputArr[1];
    ...
    if (doc.containsKey("output") && doc["output"].is<JsonArray>()) {
        JsonArray outputArr = doc["output"];
        if (outputArr.size() >= 2) {
            settings.scale_out_min = outputArr[0];
            settings.scale_out_max = outputArr[1];
    ...
    if (doc.containsKey("points") && doc["points"].is<JsonArray>()) {
        JsonArray pointsArr = doc["points"];
        int bpIdx = 0;
        for (JsonVariant v : pointsArr) {
            if (bpIdx >= MAX_BREAKPOINTS) break;
            if (v.is<JsonArray>()) {
                JsonArray point = v.as<JsonArray>();
                if (point.size() >= 2) {
                    settings.bp_in[bpIdx] = point[0];
                    settings.bp_out[bpIdx] = point[1];
                    bpIdx++;
                }
            }
        }
...
void parseAndApplyAdvanced(String json) {
    ...
    if (doc.containsKey("ditherEnable")) {
        settings.dither_enable = (doc["ditherEnable"].as<int>() == 1);
    }
    if (doc.containsKey("ditherFreq")) {
        settings.dither_freq = doc["ditherFreq"];
    }
    if (doc.containsKey("ditherAmplitude")) {
        settings.dither_ampl = doc["ditherAmplitude"];
    }
...
void setNeoColor(uint8_t r, uint8_t g, uint8_t b) {
    uint32_t color = statusNeoPixel.Color(r, g, b);
    if (color == currentNeoColor) {
        return;
    }
    currentNeoColor = color;
    statusNeoPixel.setPixelColor(0, color);
    statusNeoPixel.show();
}

void turnNeoOff() {
    if (currentNeoColor == 0) {
        return;
    }
    currentNeoColor = 0;
...
void setupBLE() {
    ...
    snprintf(bleDeviceName, sizeof(bleDeviceName), "%s_%02X%02X", FW_VARIANT, mac[4], mac[5]);
...
void loadSettings() {
    ...
    if(size == sizeof(Configuration)) {
        ...
        if(settings.magic_number == MAGIC_ID) {
            ...
            if (settings.bp_count < 0) settings.bp_count = 0;
            if (settings.bp_count > MAX_BREAKPOINTS) settings.bp_count = MAX_BREAKPOINTS;
...
void updateStatusLed() {
    const bool saturationActive = runtime.inputPhysical_global >= settings.scale_in_max;
    const bool pwmMaxActive = runtime.duty_cycle >= (Constants::DUTY_MAX - 0.0001f);
    unsigned long now = millis();

    if (pwmMaxActive) {
        bleBluePulseActive = false;
        setNeoColor(255, 0, 0); // red for duty-max
        bleBlueLastCycleMs = now;
        return;
    }

    if (saturationActive) {
        bleBluePulseActive = false;
        setNeoColor(255, 128, 0); // orange for saturation
        bleBlueLastCycleMs = now;
        return;
    }

    if (deviceConnected) {
        if (bleBlueLastCycleMs == 0UL) {
            bleBlueLastCycleMs = now;
            bleBluePulseActive = false;
            turnNeoOff();
            return;
        ...
        if (bleBluePulseActive) {
            if (now - bleBluePulseStartMs >= 1000UL) {
                bleBluePulseActive = false;
                turnNeoOff();
                bleBlueLastCycleMs = now;
            }
            ...
            turnNeoOff();
            if (now - bleBlueLastCycleMs >= 60000UL) {
                bleBluePulseActive = true;
                bleBluePulseStartMs = now;
                setNeoColor(0, 255, 0); // green heartbeat while connected
            }
        }
        return;
    }
...
void resetDitherState() {
    ...
    runtime.current_dither = 0;
    runtime.lastLedUpdateUs = 0;
    runtime.led_breath_phase = 0;
}
...
float computeDither() {
    ...
    float angle_addition = 2.0 * M_PI * settings.dither_freq * deltaTime;
    runtime.dither_angle = fmod(runtime.dither_angle + angle_addition, 2.0 * M_PI);
...
float updatePIDController(float Icmd, float Iout) {
    ...
    if (runtime.faultStatus) {
        runtime.integral = 0.0f;
    } else if(runtime.duty_p > Constants::DUTY_MIN && runtime.duty_p < Constants::DUTY_MAX) {
